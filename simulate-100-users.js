...
// Run this with: node simulate-100-users.js
...
/**
 * Gets a token by calling Firebase Authentication REST API anonymously
 * This replaces the IndexedDB method, using the apiKey from asd.js
 */
...
/**
 * Listen to data/characters in the database using Server-Sent Events (SSE)
 */
async function connectUser(userId, token) {
    ...
    // Consume the stream in the background without blocking the connection loop
    (async () => {
        try {
            ...
            while (true) {
                ...
                // Feel free to uncomment the console.log below to see the SSE payloads
                // console.log(`User ${userId} payload:`, chunk);
            }
        } catch (error) {
...
async function runSimulation() {
    ...
    // Loop until we reach 100 active connections
    while (activeConnections < NUM_USERS) {
        const promises = [];
        // Try to blast as many connections as we need to reach the target simultaneously
        ...
        try {
            ...
            // If Promise.all succeeds, we connected everyone without a 402 error!
        } catch (error) {
